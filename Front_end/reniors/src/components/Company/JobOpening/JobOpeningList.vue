<template>
  <div>
    <div class="company-jobOpening-btn-box">
      <div @click="changeing()">
        <div class="number" :class="{'now-tab' : this.tab == 'ing'}">{{ jobopeninglisting.length }}</div>
        <div>채용 중 </div>
      </div>

      <div @click="changeed()">
        <div class="number" :class="{'now-tab' : this.tab == 'ed'}">{{ jobopeninglisted.length }}</div>
        <div>채용 완료</div> 
      </div>      
    </div>
  </div>  
  ...
  <button class="create-jobOpening-btn" @click="regjobopening()">채용 등록하기</button>
</template>
