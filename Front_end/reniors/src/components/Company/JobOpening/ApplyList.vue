<template>
  <div class="company-jobOpening-info-box">
    ...
    <div class="apply-number">지원자 수  <p>{{ jobopeningdetail.applies }}</p>명</div>
  </div>
    <div class="tab-btn-box">
      <span class="tab-btn" :class="{'now-tab' : this.progress == '서류심사중'}" @click="change('서류심사중')">서류</span>
      <span class="tab-btn" :class="{'now-tab' : this.progress == '면접심사중'}" @click="change('면접심사중')">면접</span>
      <span class="tab-btn" :class="{'now-tab' : this.progress == '최종합격'}" @click="change('최종합격')">합격</span>
    </div>
